...
const FROM_EMAIL = 'FormEasy <noreply@formeasy.in>';
...
// Send email via Resend (falls back to console.log if no API key)
  ...
  <div style="max-width:600px;margin:0 auto;padding:20px;">
    ...
    <div style="background:#fff;padding:30px;border-radius:0 0 12px 12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
      ...
      <p style="color:#9CA3AF;font-size:12px;text-align:center;margin:0;">
        This is an automated email from FormEasy. Please do not reply.<br>
...
export async function notifyApplicationSubmitted(
  ...
  return sendEmail({
    ...
    html: baseTemplate(
      ...
      <p style="color:#374151;font-size:15px;">You can track your application from your dashboard.</p>
      <a href="https://formeasy.in/dashboard" style="display:inline-block;background:#1B2559;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin:16px 0;">View Dashboard →</a>
      <p style="color:#374151;font-size:15px;">Thank you for choosing FormEasy! 🎯</p>
      `
    ),
  });
}
...
export async function notifyPaymentConfirmed(
  ...
  return sendEmail({
    ...
    html: baseTemplate(
      ...
      <p style="color:#374151;font-size:15px;">Our team will start working on your form shortly.</p>
      <a href="https://formeasy.in/dashboard" style="display:inline-block;background:#1B2559;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin:16px 0;">Track Application →</a>
      `
    ),
  });
}
...
export async function notifyStatusChanged(
  ...
  const statusConfig: Record<string, { color: string; bg: string; border: string; emoji: string; message: string }> = {
    IN_PROCESS: {
      color: '#92400e', bg: '#fffbeb', border: '#fde68a', emoji: '🔍',
      message: 'Your application is now being reviewed by our team.',
    },
    FORM_FILLED: {
      color: '#1e40af', bg: '#eff6ff', border: '#bfdbfe', emoji: '📝',
      message: 'Your form has been filled and submitted on the official portal.',
    },
    COMPLETED: {
      color: '#166534', bg: '#f0fdf4', border: '#bbf7d0', emoji: '🎉',
      message: 'Your application has been completed successfully!',
    },
    REJECTED: {
      color: '#991b1b', bg: '#fef2f2', border: '#fecaca', emoji: '❌',
      message: 'Your application could not be processed. Please contact support.',
    },
  };

  const config = statusConfig[newStatus] || {
    color: '#374151', bg: '#f9fafb', border: '#e5e7eb', emoji: '📋',
  ...
  return sendEmail({
    ...
    html: baseTemplate(
      ...
      <a href="https://formeasy.in/dashboard" style="display:inline-block;background:#1B2559;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin:16px 0;">View Details →</a>
      `
    ),
  });
}
...
export async function notifyWelcome(
  ...
  return sendEmail({
    ...
    html: baseTemplate(
      ...
      <a href="https://formeasy.in/exams" style="display:inline-block;background:#1B2559;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;margin:16px 0;">Browse Forms →</a>
      <p style="color:#374151;font-size:15px;">Need help? Reply to this email or visit our <a href="https://formeasy.in/contact" style="color:#1B2559;">contact page</a>.</p>
      `
    ),
  });
}
