...
export function PdfExport({
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      doc.text(
        `Generated by CyberSeva on ${new Date().toLocaleDateString('en-IN')} | This is a computer-generated document`,
        margin,
        footerY
      );

      doc.save(`CyberSeva-${examTitle.replace(/\s+/g, '-')}-${applicantName.replace(/\s+/g, '-')}.pdf`);
    } catch (error) {
