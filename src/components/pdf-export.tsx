...
export function PdfExport({
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      doc.text('FormEasy', margin, 20);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text('EVERY FORM. ONE PLATFORM.', margin, 28);
      doc.setFontSize(9);
      ...
      doc.text(
        `Generated by FormEasy on ${new Date().toLocaleDateString('en-IN')} | This is a computer-generated document`,
        margin,
        footerY
      );

      doc.save(`FormEasy-${examTitle.replace(/\s+/g, '-')}-${applicantName.replace(/\s+/g, '-')}.pdf`);
    } catch (error) {
